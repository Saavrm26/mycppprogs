...
bool vis[8][8];
int lev[8][8];
pii mov[] = {{1,2},{1,-2},{2,1},{2,-1},{-2,1},{-2,-1},{-1,2},{-1,-2}};
int getX(string s){
    return s[0] - 'a';
}
int getY(string s){
...
void bfs(int sX,int sY){
    ...
    while(!q.empty()){
        ...
        q.pop();
        ff(i,0,7){
            if(!isValid(par.fi + mov[i].fi,par.se + mov[i].se)) continue;
            if(vis[par.fi + mov[i].fi][par.se + mov[i].se]) continue;
            q.push({par.fi + mov[i].fi,par.se + mov[i].se});
            lev[par.fi + mov[i].fi][par.se + mov[i].se]+=lev[par.fi][par.se]+1;
            vis[par.fi + mov[i].fi][par.se + mov[i].se]=1;
        }
    }
}
void solve(){
