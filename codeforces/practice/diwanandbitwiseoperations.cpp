...
//  ll power_of_2(ll n,ll curr){ ll val = 1<<curr; if(n%val==0) return curr +
//  power_of_2(n/val,curr+1); else{ if(n%2==0){ return 1+power_of_2(n/2,2); }
//  return 0; } } ld prec = 1e-7; bool iseq(ld v1,ld v2){ return abs(v2 - v1)<=
//  prec; } bool islt(ld v1,ld v2){if(iseq(v1,v2)) return 0; return v1<v2;} bool
//  isgt(ld v1,ld v2){if(iseq(v1,v2)) return 0; return v1>v2;} bool isgte(ld
//  v1,ld v2){if(iseq(v1,v2)) return 1; return v1>v2;} bool islte(ld v1,ld
...
ll xorSum(vll arr, int n) {
  ll bits = 0;
  for (int i = 0; i < n; ++i)
    bits |= arr[i];

  ll ans = ((bits % mod) * binpow(2, n - 1)) % mod;
  return ans;
}
