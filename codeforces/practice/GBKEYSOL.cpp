...
void solve(){
    ...
    ff(i,0,n-1){
        dp[i+1][0]=dp[i][0] + v[i] -k;
        ff(x,0,29){
            if(x<=i){
                // dp[i + 1][x + 1] = max(dp[i + 1][x + 1], (dp[i][x] + (v[i] / ll(1LL << (x * 1LL + 1)))));
                dp[i + 1][x + 1] = max(dp[i + 1][x + 1], (dp[i][x] + (v[i] >> (x * 1LL + 1))));
                dp[i + 1][x] = max(dp[i + 1][x], dp[i][x] + (v[i] >> (min(x,i) * 1LL)) - k);
            }
            else{
                dp[i + 1][x + 1]=0;
                dp[i+1][x]=0;
            }
            // dp[i + 1][x] = max(dp[i + 1][x], dp[i][x] + v[i] / ll(1LL << (x * 1LL)) - k);
            // dp[i + 1][x] = max(dp[i + 1][x], dp[i][x] + (v[i] >> (min(x,i) * 1LL)) - k);
        }
    }
    ll ans=INT64_MIN;
