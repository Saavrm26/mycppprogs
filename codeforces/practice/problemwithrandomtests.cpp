...
void solve(){
    ...
    ff(i,0,n-1){
        if(s[i]=='1'){
            actual_start = i;break;
        }
    }
    string first_subarray = "";
    ff(i,actual_start,n-1){
        first_subarray += s[i];
    }
    int N1 = first_subarray.size();
    int ctr = 0;
    ff(i,0,N1-1){
        if(first_subarray[i]!='0'){
            ctr++;
        }
        else{
            break;
    ...
    string ans= 0;
    for(int i =0;i<=ctr;i++){
        string fcopy = first_subarray;
        string second_subarray = first_subarray.substr(0,N1-i);
        int N2 = second_subarray.size();
        int j=N1-1,k=N2-1;
        for(;k>=0;k--){
            if(fcopy[j]=='1'||fcopy21[k]=='1'){
                first_subarray[j] = '1';
            }
        }
    }
