...
pair<ll,ll> calc(int i,int o,int e){
    if(i==n){
        if(n&1){
            if(o<0&&e==0){
                return mp(even[e],0);
    ...
    if(i&1){
        //alice
        if(e>=0){
            auto temp=calc(i+1,o,e-1);
            return mp(temp.fi+even[e],temp.se);
        }
        if(o>=0){
            auto temp=calc(i+1,o-1,e);
            return mp(temp.fi,temp.se);
        }
    }
    else{
        //bob
        if(e>=0){
            ...
            auto temp=calc(i+1,o-1,e);
            return mp(temp.fi,temp.se+odd[o]);
        }
    }
