...
void sample(int i1,int j1,int i2,int j2){
    if((i1<=r&&r<=i2)&&(j1<=c&&c<=j2)){
        int ii=r,jj=c;
        ff(i,1,k){
            sam[ii][jj]='X';
            ii++;jj++;
...
void loops(int i1,int j1,int i2,int j2){
    for(int ii=i1,kk=1;kk<=k;kk++,ii++){
        for(int jj=j1,kkk=1;kkk<=k;kkk++,jj++){
            mat[ii][jj]=sam[kk][kkk];
        }
    }
}
void solve(){
    ...
    if(k!=1) str.append(nn,'.');
    else str.append(nn,'X');
