...
void solve(){
    ini(n) ini(t1) ini(t2) ini(k)
    vector<pair<dbll,int>> vec(n);
    k=100-k;
    ff(i,0,n-1){
        ini(r1) ini(r2)
        dbll h1 = t1*(dbll(k)/100)*r1 + t2*r2;
        dbll h2 = t1*(dbll(k)/100)*r2 + t2*r1;
        dbll maxi = fmax(h1,h2);
        vec[i]={maxi,i+1};
    }
    float prec=1e-2;
    sort(all(vec),[&](auto h1,auto h2){
        if(fabs(h1.fi - h2.fi)<=prec){
            return h1.se<h2.se;
        }
        else{
            return h1.fi>h2.fi;
