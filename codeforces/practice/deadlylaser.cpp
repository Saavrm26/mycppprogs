...
bool is_valid(int i,int j){
    ...
    if(dist(i,j,sx-1,sy-1)<=d) return 0;
    return 1;
}
int main(){
    #ifndef ONLINE_JUDGE
...
void solve(){
    fastIO;
    cin>>n>>m>>sx>>sy>>d;
    bool sx1=1,sxm=1,_1sy=1,nsy=1;
    if(dist(sx,1,sx,sy)<=d){
        sx1=0;
    }
    if(dist(sx,m,sx,sy)<=d){
        sxm=0;
    }
    if(dist(1,sy,sx,sy)<=d){
        _1sy=0;
    }
    if(dist(n,sy,sx,sy)<=d){
        nsy=0;
    }
    if(((sx1&&nsy)||(sxm&&_1sy))){
        cout<<(n-1+m-1)<<"\n";
        ...
        cout<<-1<<"\n";
    // ff(i,1,n){
    //     if(dist(1,i,sx,sy)<=d){f1=0;break;}
    // }
    // ff(i,1,m){
    //     if(dist(i,m,sx,sy)<=d){f1=0;break;}
    // }
    // ff(i,1,n){
    //     if(dist(i,1,sx,sy)<=d){f2=0;break;}
    // }
    // ff(i,1,m){
    //     if(dist(n,i,sx,sy)<=d){f2=0;break;}
    // }
    // if(f1||f2){
    //     cout<<(n-1+m-1)<<"\n";
    // }
