...
ll findSqrt(ll n)
{
    ...
    while (l<=r)
    {
        ll mid = l + (r-l)/2;
        ll curr= mid*mid;
        ll upper = (mid+1)*(mid+1);
        ll lower = (mid-1)*(mid-1);
        if(curr<=n&&upper>n){
            return mid;
        }
        else if(curr>n&& lower<=n){
            return mid-1;
        }
        else if(curr>n){
            r = mid -1;
...
void solve(){
    inll(l) inll(r)
    ll r1=findSqrt(l),r2=findSqrt(r);
    ll ans=0;
    ll new_l = (r1+1)*(r1+1);
    ll new_r = new_r=(r2)*(r2);
    ll new_lr=r1+1,new_rr=r2;
    if(new_l>=r){
        ans+= (r-1)/r1 - (l-1)/r1;
        if(r%r2==0) ans++;
    }
