...
int main(){
    ...
    while(t--){
        ...
        init2d(0,vis,N-1,N-1);
        init2d(0,dis,N-1,N-1);
    }
}
...
void solve(){
    ...
    dis[n-1][m-1]=1;
    if(v[n-1][m-1]!='#') bfs(n-1,m-1);
    ff(i,0,n-1){
        ff(j,0,m-1){
            if(v[i][j]=='B' && dis[i][j]!=0){
                no;return;
            }
            else if(v[i][j]=='G' && dis[i][j]==0){
                no;return;
            }
        }
    }
