...
#define ordered_set tree<pair<long long,pair<long long,long long>>, null_type,less<pair<long long,pair<long long,long long>>>, rb_tree_tag,tree_order_statistics_node_update>
...
void solve(){
    ...
    ff(i,1,N){
        ff(j,1,N){
            pre[i][j]=pre[i-1][j]+grid[i][j]+pre[i][j-1]-pre[i-1][j-1];
        }
    }
    while(q--){
        ini(hs) ini(ws) ini(hb) ini(wb)
