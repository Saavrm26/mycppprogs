...
void solve(){
    ...
    if(v[0]&1){
        //first term is odd
        ff(i,0,o-1){
            // last odd term ko use karke saare odd terms ko equal karna
            if((v[i]&1))
                cout<< min(i+1,o+1) <<" "<< max(i+1,o+1) <<"\n";
        }
        ff(i,1,n-1){
            // first term ko use karke saare even terms ko odd karna
            if(!(v[i]&1))
                cout<< min(1,i+1) <<" "<< max(1,i + 1) <<"\n";
        }
    }
    else{
        //first term is even
