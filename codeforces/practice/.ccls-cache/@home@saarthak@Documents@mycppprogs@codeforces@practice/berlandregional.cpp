...
void solve(){
    ...
    ff(i,0,n-1) bst[s[i]].push_back(u[i]);
    ff(i,0,n-1) sort(bst[i].begin(), bst[i].end(), greater<int>());
    vvll pr(n,vll(1,0));
    // ff(i,0,n-1){
    //     pr[i].emplace_back(0);
    // }
    ff(i,0,n-1){for (int x : bst[i]) pr[i].push_back(pr[i].back() + x);}
    vector<long long> ans(n);
    ff(i,0,n-1) for (int k = 1; k <= int(bst[i].size()); ++k)
