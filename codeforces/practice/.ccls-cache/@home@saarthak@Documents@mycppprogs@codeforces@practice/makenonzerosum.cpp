...
//  ll binpow(ll a, ll b) {ll res = 1;while (b > 0) {if (b & 1) res = res * a;a
//  = a * a;b >>= 1;}return res;} ll power_of_2(ll n,ll curr){ ll val = 1<<curr;
//  if(n%val==0) return curr + power_of_2(n/val,curr+1); else{ if(n%2==0){
//  return 1+power_of_2(n/2,2); } return 0; } } ld prec = 1e-7; bool iseq(ld
//  v1,ld v2){ return abs(v2 - v1)<= prec; } bool islt(ld v1,ld
//  v2){if(iseq(v1,v2)) return 0; return v1<v2;} bool isgt(ld v1,ld
//  v2){if(iseq(v1,v2)) return 0; return v1>v2;} bool isgte(ld v1,ld
//  v2){if(iseq(v1,v2)) return 1; return v1>v2;} bool islte(ld v1,ld
//  v2){if(iseq(v1,v2)) return 1; return v1<v2;}
template <typename T, typename U> U slicing(T const &v, int X, int Y) {
...
template <class T>
using omm = tree<T, null_type, less_equal<T>, rb_tree_tag,
                 tree_order_statistics_node_update>;
#define ook order_of_key
#define fbo find_by_order
template <typename F> void deb(F &&lamda) {
#ifndef ONLINE_JUDGE
    lamda();
#endif
}
...
int main() {
    ...
    deb([] {
        cerr << "Runtime is: " << (clock() * 1.0 / CLOCKS_PER_SEC) * 1000
             << "ms\n";
    });
}
void solve() {
