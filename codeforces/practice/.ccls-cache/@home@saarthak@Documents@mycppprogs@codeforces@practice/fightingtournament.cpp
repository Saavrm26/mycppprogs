...
void solve(){
    ...
    ff(i,0,n-2){
        if(v[p1]<v[p2]){
            victories[v[p2]]++;
            vic2[i]=mp(v[p2],victories[v[p2]]);
            p1=p2+1;
        }
        else{
            victories[v[p1]]++;
            vic2[i]=mp(v[p1],victories[v[p1]]);
            p2=p1+1;
        }
    }
    // vic2.insert(vic2.begin(),vic2[0]);
    ff(i,0,q-1){
        ini(pos) ini(k)
        if(k<=(n-1)){
            if(k==pos-1){
                if(v[pos-1]==vic2[k-1].fi)
                    cout<<vic2[k-1].se<<"\n";
                else if(v[pos-1]<vic2[k-1].fi){
                    cout<<victories[v[pos-1]]<<"\n";
                }
            }
            else if(pos-1>k){
                cout<<0<<"\n";
            ...
            if(v[pos-1]!=maxi){
                ...
                cout<<victories[v[pos-1]]+(k-(n-1))/(n-1) + k%(n-1)<<"\n";
            }
        }
    }
}