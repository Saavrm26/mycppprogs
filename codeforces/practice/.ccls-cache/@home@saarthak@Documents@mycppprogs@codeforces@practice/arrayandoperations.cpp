...
void solve(){
    ...
    auto v1=slicing<vll,vll>(v,1,n-2*k);
    auto v2=slicing<vll,vll>(v,n-2*k+1,n);
    // trace1d(v1,int(v1.size()-1));
    int n2=v2.size();
    v2.insert(v2.begin(),1e9+1);
    // trace1d(v2,int(v2.size()-1));
    auto v3=slicing<vll,vll>(v2,1,n2/2);
    auto v4=slicing<vll,vll>(v2,(n2)/2+1,n2);
    // trace1d(v3,int(v3.size()-1));
