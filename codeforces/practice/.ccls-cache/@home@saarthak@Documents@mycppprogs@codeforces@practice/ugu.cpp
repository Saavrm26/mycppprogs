...
void solve(){
    ...
    ins(s)
    auto str = s;
    int flips1 = 0,flips2=1;
    ff(i,1,n-1){
        if(flips1&1){
            if(s[i-1]==s[i]&&s[i]=='1') flips1++;
            else{
                if(s[i]=='0') s[i]='1';
                else s[i] = '0';
    ...
    ff(i,1,n-1){
        if(flips2&1){
            ...
            if(s[i-1]>s[i]) {flips2++;s[i]=s[i-1];}
        }
    }
    cout<<min(flips2,flips1)<<"\n";
}