...
void solve() {
	...
	s_p.insert(s_p.begin(), 0);
	x_p.insert(x_p.begin(), 0);
	while(q--){
		ini(L) ini(R);
		int ansl=L,ansr = R;					
		ll curr  = s_p[R] - s_p[L-1] - (x_p[R]^x_p[L-1]);
		for(int r = L;r<=R;r++){
			int currl=-1,currr=r;
			//binary serach on l
			int hi = r,lo = L;
			while(hi-lo>1){
				int mid = (hi+lo)/2;
				ll val = s_p[r] - s_p[mid-1] - (x_p[r]^x_p[mid-1]);
				if(val==curr) {
					currl=mid;
					lo = mid;
				}
			...
			if(hi-lo<=1){	
				if(s_p[r] - s_p[lo-1] - (x_p[r]^x_p[lo-1])==curr){
					currl=lo;
				}
				if(s_p[r] - s_p[hi-1] - (x_p[r]^x_p[hi-1])==curr){
					currl = hi;
				}
			}
			if(currl!=-1 && currr - currl +1 < ansr - ansl + 1){
				ansl=currl;ansr=currr;
			}
		}	
		cout<<ansl<<" "<<ansr<<"\n";
	}	
