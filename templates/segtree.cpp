...
class segtree{
    public:
        ...
        void update(int pos,int val,int i,int s,int e);
};

void segtree::build(int i,int l,int r){
    int mid = l + (r - l) / 2;
    if (l == r){
        Tree[i] = gArr[l];
        return;
...
int segtree::query(int i,int s,int e,int l,int r){
    if(l<=s&&e<=r){
        return Tree[i];
    }
    if(r<s||l>e){
    ...
    int mid=(e+s)/2;
    int q1=query(2*i+1,s,mid,l,r);
    int q2=query(2*i+2,mid+1,e,l,r);
    return max(q1,q2);
}
void segtree::update(int pos,int val,int i,int s,int e){
    if(s==e){
        gArr[pos]=val;
        Tree[s]=val;
        return;
    ...
    int mid=(s+e)/2;
    int c1=2*i+1,c2=2*i+2;
    if(s<=pos&&pos<=mid) update(pos,val,c1,s,mid);
    else update(pos,val,c2,mid+1,e);
    Tree[i]=max(Tree[c1],Tree[c2]);
}
...
void solve(){
    ...
    segtree stree(arr,n);
    trace1d(stree.gArr,n);
    stree.build(0,0,n-1);
    deb(
        [&]{
            trace1d(stree.Tree,4*n);
        }
    );
    while(q--){
        ini(c)
        if(c){
            ini(l) ini(r)
            cout<<stree.query(0,0,n-1,l-1,r-1)<<"\n";
        }
        else{
            ini(pos) ini(val)
            stree.update(pos-1,val,0,0,n-1);
            arr[pos-1]=val;
