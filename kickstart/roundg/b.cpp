...
void solve(){
    ...
    sort(all(Rd));
    if(Yd.empty()){
        auto it = --ub(all(Rd),ld(rh));
        cout<<(it - Rd.begin() + 1)<<" "<<0<<"\n";
    }
    else if(Rd.empty()){
        auto it = --ub(all(Yd),ld(rh));
        cout<<0<<" "<<(it - Yd.begin() + 1)<<"\n";
    }
    else if(Yd[0]<Rd[0]){
        auto it = --ub(all(Yd),min(ld(rh),ld(Rd[0])));
        cout<<0<<" "<<(it - Yd.begin() + 1)<<"\n";
    }
    else{
        auto it = --ub(all(Rd),min(ld(rh),ld(Yd[0])));
        cout<<(it - Rd.begin() + 1)<<" "<<0<<"\n";
    }
}