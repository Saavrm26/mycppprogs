...
void solve();
map<char,function<pii(int,int)>> F;
int n,m;
...
int main(){
    ...
    F['U'] = [&](int i, int j) -> pii
    {
        if (!(i - 1 < 1)) return mp(i - 1, j) ;else return  mp(-1, -1);
    };
    F['D'] = [&](int i, int j) -> pii
    {
        if (!(i + 1 > n)) return mp(i + 1, j) ;else return mp(-1,-1);
    };
    F['R'] = [&](int i, int j) -> pii
    {
        if (!(j + 1 > m)) return mp(i, j + 1) ;else return mp(-1, -1);
    };
    F['L'] = [&](int i, int j) -> pii
    {
        if (!(j - 1 < 1)) return mp(i, j - 1) ;else return  mp(-1, -1);
    };
    solve();
    deb([]{
...
void recurse(int i,int j){
    ...
    auto fun = (F[grid[i][j]]);
    auto val =fun(i,j);
    if(val.fi==-1||val.se==-1){
        cout<<i<<" "<<j<<"\n";
        return;
    }
    recurse(val.fi,val.se);
}
void solve(){
    cin>>n>>m;
    grid.resize(n+1);vis.resize(n+1);
    ff(i,1,n){
        grid[i].resize(m+1);vis[i].resize(m+1);
    }
    ff(i,1,n){
        ff(j,1,m){
            cin>>grid[i][j];
