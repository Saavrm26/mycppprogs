...
int main(){
    ...
    while(t--){
        ...
        ff(i,0,n-1){
            if(m.find(v[i])==m.end()){
                m[v[i]] =i+1;
                // mp(max(i + 1 - v[i] + 1, 1), min(i + 1 + v[i] - 1, n));
                ll tmp;
                if(v[i]<=(i+1)&&v[i]<=(n-i)){
                    tmp=v[i];
                }
                else if(v[i]>=(i+1)&&v[i]>=(n-i)){
                    tmp=1;
                }
                else{
                    tmp=min((i+1-0),(n-i));
                ...
                pii curr = mp(max(i + 1 - v[i] + 1, 1), min(i + 1 + v[i] - 1, n));
                auto prev=mp(max(m[v[i]] - v[i] + 1, 1), min(m[v[i]] + v[i] - 1, n));
                ll tmp;
                if(v[i]<=(i+1)&&v[i]<=(n-i)){
                    tmp=v[i];
                }
                ...
                if (prev.fi <= i+1 && i+1<=prev.se){
                    // tmp =min(v[i],min((i+1-0),(n-i)))-( m[v[i]].fi-curr.fi);
                    // tmp=( m[v[i]].fi-curr.fi)+1;
                    tmp-=(i+1-m[v[i]]);
                }
